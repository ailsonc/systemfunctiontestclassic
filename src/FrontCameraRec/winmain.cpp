...
namespace MainWindow
{
    ...
    void UpdateUI(HWND hwnd)
    {
        if (g_pEngine->IsRecording() != bRecording)
        {
            ...
            if (bRecording)
            {
				SendMessage(hRecordwButton, WM_SETTEXT, 0, (LPARAM)L""); // Stop sign
                //SetMenuItemText(GetMenu(hwnd), ID_CAPTURE_RECORD, L"Stop Recording");
            }
            else
            {
				SendMessage(hRecordwButton, WM_SETTEXT, 0, (LPARAM)L""); // Video sign
	...
	int Camera_Select(int iCamera)
	{
		...
		for (DWORD i = 0; i < param.count; i++)
		{
			...
			else if (szFriendlyName != NULL)
			{
				...
				{
					iCamera_Panel = QueryPLD(szFriendlyName, Front_Camera);
				}
				CoTaskMemFree(szFriendlyName);
		...
		hr = pSelectedDevice->ActivateObject(
	...
	int QueryPLD(WCHAR * wszFriendlyName, int iCameraFront)
	{
		HDEVINFO hDevInfo;
	...
	void SetAutoFocusSupport()
	{
		if (pMediaSource == NULL)
			return;

		HRESULT hr = S_OK;
		IAMCameraControl *pCamControl = NULL; //IAMVideoProcAmp 
		hr = pMediaSource->QueryInterface(IID_PPV_ARGS(&pCamControl));
		...
		if (hr == S_OK) {
			...
			pCamControl->Get(CameraControl_Focus, &gValue, &gFlags);

			if (gFlags != CameraControl_Flags_Auto)
			{

				hr = pCamControl->Set(CameraControl_Focus, // property
					gValue, // value
					CameraControl_Flags_Auto //Flag
					);
	...
	void OnStartRecord(HWND hwnd)
	{
		...
		hr = pFileSave->SetFileName(L"videotest.mp4");
		if (FAILED(hr))
		{
		goto done;
		}
		...
		hr = pItem->GetDisplayName(SIGDN_FILESYSPATH, &pszFileName);//C:\Users\v-alvcha\Desktop\videotest.mp4
		...
		*/
		...
		if (!GetModuleFileNameA(NULL, absolutepath, sizeof(absolutepath)))
		{
			goto done;
		}
		PathRemoveFileSpecA(absolutepath);
		swprintf(filepath, L"%S", absolutepath);


		//default encoding format is wmv
		hr = StringCchPrintf(filename, MAX_PATH, L"videoTest.wmv");
		if (FAILED(hr))
		{
			goto done;
		}
		...
		LPTSTR path = PathCombine(PhotoFileName, filepath, filename);
		if (path == NULL)
		{
			hr = E_FAIL;
			goto done;
		...
		//Strat to record video and save it
		hr = g_pEngine->StartRecord(path);
		if (FAILED(hr))
		{
			goto done;
		}
	...
	{
		//Enable auto focus if camera spport
		SetAutoFocusSupport();
    ...
    BOOL OnCreate(HWND hwnd, LPCREATESTRUCT lParam)
    {
		...
		SendMessage(hRecordwButton, WM_SETFONT, WPARAM(hFont), TRUE);
		...
		iStatus = Camera_Select(Front_Camera);
		if (iStatus)
		{
				iStatus_g = 999;
    ...
    {        
        if (state == SIZE_RESTORED || state == SIZE_MAXIMIZED)
        {
			...
			MoveWindow(hRecordwButton, (cx / 20) * 19, 0, cx / 20, cy, TRUE);
