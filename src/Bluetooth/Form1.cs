    ...
    public partial class TestBluetooth : Form
    {
        ...
        public TestBluetooth()
        {
            ...
            if (Program.ProgramArgs != null)
            {
                ...
                {
                    TestDuration = Int32.Parse(Program.ProgramArgs[0].ToString(), CultureInfo.InvariantCulture);
                }
                if (Program.ProgramArgs.Length >= 2)
                {
                    if (Program.ProgramArgs[1] != "NULL")
                    {
                        _BTPara = new string[Program.ProgramArgs.Length - 1];
                        Array.Copy(Program.ProgramArgs, 1, _BTPara, 0, Program.ProgramArgs.Length - 1);
                    }
                }
            }
