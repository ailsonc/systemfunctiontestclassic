...
namespace DllComponent {
	...
	public value struct BatteryInfo sealed
	{
		INT32 Result;
		ULONG PowerState;
		ULONG Capacity;
    ...
    public:
		...
		WINBIO_SESSION_HANDLE BioOpen();
		HRESULT BioLocate(WINBIO_SESSION_HANDLE sessionHandle);
		HRESULT TestBiometric(WINBIO_SESSION_HANDLE sessionHandle);
		HRESULT DiscardEnroll(WINBIO_SESSION_HANDLE sessionHandle);
