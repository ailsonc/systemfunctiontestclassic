...
namespace HardwareButtonTest
{
    public partial class MainForm : Form
    {
        ...
        {
            if (Program.ProgramArgs == null || Program.ProgramArgs.Length < 2) return;

            BtnDownNum = Int32.Parse(Program.ProgramArgs[0]);
            string[] ButtonList = new string[Program.ProgramArgs.Length - 1];
            Array.Copy(Program.ProgramArgs, 1, ButtonList, 0, Program.ProgramArgs.Length - 1);

            for (int i = 0; i < ButtonList.Length; i++)
            {
                Label newLabel = new Label();
                newLabel.Name = ButtonList[i];
                newLabel.Text = ButtonList[i] + " - " +BtnDownNum;
        ...
        private void MainForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                string tempControlName = k.ToString();
                ...
                {
                    Control c = _control[0];
                    List < string> tempList = BtnControls.FindAll(s => s.Equals(tempControlName));
                    if (tempList != null)
                    {
                        c.Text = tempControlName + " - " + tempList.Count;
                    }
                }
            }
        ...
        {
            Title.Text = LocRM.GetString("Keypad") + LocRM.GetString("Test");
