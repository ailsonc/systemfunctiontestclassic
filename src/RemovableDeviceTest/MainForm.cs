...
namespace RemovableDeviceTest
{
    public partial class MainForm : Form
    {
        ...
        private void watcher_EnumerationCompleted(DeviceWatcher sender, object args)
        {
            ...
            if (Program.ProgramArgs.Length == 0) return;

            if (Program.ProgramArgs.Length != deviceTypeList.Count)
            {
            ...
            for (int i = 0; i < deviceTypeList.Count; i++)
            {
                ...
                if (!deviceTypeList.ContainsKey(drive))
                {
                    Program.ExitApplication(255);
                    return;
                }
            ...
            try
            {
                ...
                for (int i = 0; i < allDrives.Length; i++)
                {
                    ...
                    if (d.IsReady == true)
                    {
                        ...
                        if(files.Length > 0)
                        {
                            MessageBox.Show($"Por favor, formatar o disco {d.Name} !!!","Alerta!!!",MessageBoxButtons.OK, MessageBoxIcon.Hand);
                            Log.LogError($"O pen drive {d.Name} contém arquivos maliciosos!");
                            Program.ExitApplication(255);
                        }
