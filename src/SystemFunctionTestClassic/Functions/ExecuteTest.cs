...
namespace win81FactoryTest.Functions
{
    ...
    static class ExecuteTest
    {
        ...
        public static bool BTRadio(bool enable)
        {
            string radiofile = "RadioManagers.exe";
            string args = "on";
            bool result = false;

            if (!File.Exists(radiofile))
            {
                Log.LogError("RadioManagers.exe file not found");
                return result;
            }
            ...
            {
                Log.LogError("BTRadio: Failed - Exception Caught: " + e);
                return false;
            }
            finally
            {
